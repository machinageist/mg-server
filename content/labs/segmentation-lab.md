---
title: "S2 — Prove LAB (VLAN 50) first"
date: 2026-08-14
...
1. **Trunk the host port** for the node hosting your chosen lab guest: native
   or untagged management, plus tagged VLAN 50.
2. **Make that node's bridge VLAN-aware.** One node only. Leave the others
   alone until this one is proven.
3. **Enable VLAN 50 on the firewall**, with DHCP or static routing as you
   prefer.
4. **Apply the default-deny LAB policy *before* moving the guest.** Not after.
...
5. **Tag one** disposable guest for VLAN 50.
...
```bash
ip -br address     # an address in the LAB range
ip route           # default route via the LAB gateway
getent hosts example.com                  # DNS resolves
curl -sSI https://example.com | head -1   # egress works
```

**Negative** — all of these must **fail**, and failing is the pass condition:

```bash
ssh <a management address>          # MGMT — must be denied
curl -sS --max-time 5 https://<a servers address>   # SERVERS — must be denied
ping -c2 <the trusted gateway>      # TRUSTED — must be denied
```
...
- [ ] VLAN 50 live on the firewall with default-deny applied **before** the
      guest moved
