...
questions:
  - stem: "How many addresses can be assigned to interfaces on a /26?"
    ...
    explanation: >
      ...
      mistake. 60 does not come from any calculation. It is what you get by
      rounding, and this arithmetic is exact, so do not round.
  ...
  - stem: "The two formulas that cover most subnetting arithmetic are 2^n and 2^n minus 2. What decides which one applies?"
    ...
    answer: 2
    explanation: >
      Mixing these up is the easiest mistake to make here. The subtraction
      exists because two host addresses are taken, the network address and the
      broadcast address, and that only applies when you are counting hosts. Counting how many subnets a set of
      ...
      the segment is sending broadcasts. The broadcast address is reserved
  ...
  - stem: "A host with address 10.10.10.10 and mask 255.255.255.0 wants to send to 10.10.20.5. What does it do with the mask, and what does it conclude?"
    ...
    explanation: >
      ...
      networks. Comparing octets by eye and stopping at two is how people
      guess, and it is exactly what the mask exists to replace. ORing with
  ...
  - stem: "What does writing 10.10.10.10/24 give you that writing 10.10.10.10 with mask 255.255.255.0 does not?"
    ...
    answer: 1
    explanation: >
      The two notations carry the same information. /24 and 255.255.255.0 are
      the same 24 leading 1s. What CIDR changed was that the boundary is stated
      explicitly instead of inferred from the first octet, so any prefix
      ...
      to be configured. The notation does not do it. And nothing about prefix
      notation makes a boundary dynamic. It is as fixed as a dotted mask.
  ...
  - stem: "You are dividing 10.10.10.0/24 for three segments needing 45, 25, and 10 hosts. Why allocate the largest first?"
    ...
    explanation: >
      ...
      Take the 10-host segment first and it uses a /28 at .0 through .15. The
      next legal /26 boundary is then .64, and the space between is stranded. Largest-first packs each block against the previous one with no
  ...
  - stem: "A segment needs 45 usable addresses. What is the shortest prefix that is not wasteful, meaning the smallest block that fits?"
    options:
    ...
    explanation: >
      ...
      space to hold the same 45 hosts, and a /24 wastes the whole block.
      Variable length masking means picking the smallest block that works, not
  ...
  - stem: "Beyond conserving addresses, what does dividing a network into subnets buy you?"
    ...
    explanation: >
      ...
      filtering policy can be applied. That is why segmentation is a security
      design tool and not only an addressing one. Bounding broadcast traffic
      comes from the same boundary. Nothing about routing encrypts anything.
      That takes a protocol built for it. Routers do reroute around failed
  ...
  - stem: "You ping your network's broadcast address and get two replies on a segment you know has a dozen hosts. What should you conclude?"
    ...
    explanation: >
      ...
      Treating ten silent hosts as down reads far more into the result than it
      supports. A wrong broadcast address usually produces no replies
  ...
  - stem: "Given the reason for learning the arithmetic by hand, what is the argument for using `ipcalc` in day-to-day work?"
    ...
    explanation: >
      ...
      slower, less reliable work. VLSM can be worked entirely by hand. That is
      what the largest-first exercise is. And every prefix length has a
