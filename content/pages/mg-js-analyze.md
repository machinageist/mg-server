...
[mg-sourcemap](/wiki/mg-sourcemap).
...
```bash
mg-js-analyze acme-bounty
mg-js-analyze acme-bounty --concurrency 10
```
...
- Secret masking applies before writing: only the first 8 characters of a matched
  value are stored.
- Source map references are collected into a list that `mg-sourcemap` reads directly.
