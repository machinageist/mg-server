...
date: 2026-08-14
summary: "Counting hosts and networks, what a subnet mask does bit by bit, prefix notation and supernetting, and sizing subnets to what each segment needs."
tags: [education, networking, subnetting, cidr, vlsm, routing]
---

## Overview

An [IPv4 address](/learn/ipv4-addressing) is 32 bits. Subnetting is deciding
where to cut those 32 bits: how many belong to the network, and how many are
left to identify hosts on it.
...
- The network address, with all host bits set to 0, names the network itself.
- The broadcast address, with all host bits set to 1, reaches every host on it.
...
Mixing up the two formulas is the easiest mistake to make here. Hosts count host
bits and subtract two. Networks count network bits and subtract nothing.
...
The host does this twice, once for its own address and once for the destination.
...
prefix length is legal, so a network can be sized to what it needs instead of
to the nearest class.

CIDR also allows supernetting, which expresses several adjacent networks as one
larger route. That keeps internet routing tables smaller than the number of
...
two. `10.1.0.0/16` and `10.2.0.0/16` cannot be combined. They are adjacent, but
`10.0.0.0/15` covers 10.0 and 10.1, not 10.1 and 10.2. The
...
what each piece needs, instead of splitting it evenly. Take `10.10.10.0/24`,
which has 256 addresses, and three segments that need 45, 25, and 10 hosts.
...
Each block starts where the previous one ended, and .112 to .255 stays free for
later. Allocating smallest first would have left the larger blocks without an
aligned boundary to start on. It is the same alignment rule as supernetting,
...
3. Work out your network's broadcast address, then confirm it with `ipcalc`.
   ...
   so a quiet result does not prove the network is empty.
4. On paper, split your own /24 with VLSM for three segments of your choosing.
   Check every boundary with `ipcalc` before trusting it, and try allocating
   smallest-first on purpose to see where the alignment fails.
...
Do the subnetting arithmetic by hand a few dozen times, then stop. `ipcalc`
exists, and using it for real work is not cheating. The reason to learn the
manual method is that it lets you spot a wrong mask on sight, and a wrong mask is
the problem you will be asked to diagnose.
