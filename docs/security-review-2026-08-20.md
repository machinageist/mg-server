...
Repository-controlled Markdown is trusted content. That trust boundary means a compromised contributor or source repository could still turn raw HTML support into stored script or markup injection. The current review strengthened CSP but did not add HTML sanitization.
...
### 5. Public copy created cumulative reconnaissance value
...
### 6. Security checks were not enforced in CI
...
- Raw HTML in repository-controlled Markdown remains a trusted-author boundary. Sanitizing or disabling raw HTML would reduce contributor and supply-chain impact.
