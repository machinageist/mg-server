...
// Static wiki sidebar layout. This is the pruned GeistScope archive — only the
// still-active safe-core pages remain (see docs/geistscope-page-triage.md).
// Every slug here MUST have a matching file in content/pages/, enforced by the
// integration test in tests/wiki_pages.rs.
...
mod tests {
    ...
    fn rendering_overview_template_includes_sidebar_and_content() {
        ...
        assert!(
            html.contains("/wiki/mg-report"),
            "sidebar should link to a kept archive page"
        );
        assert!(
        ...
        assert!(
            html.contains("Reference archive"),
    ...
    fn rendering_tool_page_marks_correct_active_entry() {
        let slug = "mg-report";
        let page = Page::find(&PathBuf::from(PAGES_DIR), slug).expect("mg-report page must exist");
        let html = WikiPageTemplate {
            page,
            sidebar: SIDEBAR,
            active_slug: slug,
        ...
        .render()
        .expect("template renders");
        // Exactly one entry is active, and its <li> wraps the mg-report link
        assert_eq!(
        ...
        assert!(
            active_li.contains("/wiki/mg-report"),
            "expected mg-report to be the active sidebar entry"
        );
    }
