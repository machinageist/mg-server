...
- *(Merged from the 4.8 spec.)* `README.md:14-16` records the same rule a third
  time, as a comment left in the repo on 2026-07-25 when the cert claims were
  pulled: name no cert until a voucher is booked, and then *"state only that
  single exam with its scheduled date."* `templates/about.html` carries the same
  comment inline. The constraint is therefore recorded in four places and has
  never been recorded as lifted — which is the strongest argument that the
  conservative reading is the intended one.
