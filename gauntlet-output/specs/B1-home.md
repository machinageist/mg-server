...
**Date:** 2026-08-08
**Iteration:** 2 (hand-merged 2026-08-14)

> **Merge note.** Commit `c176cab` merged two branches that had each run the
> gauntlet independently, and the conflict was committed unresolved — this file
> carried `<<<<<<<` markers for six days. Two different spec agents had written a
> complete spec for this feature: one on `main` (`43ef1ea`, Opus 5) and one on the
> branch (`a0bff14`, Opus 4.8).
>
> Resolved by hand in favour of the Opus 5 text, after comparing both sides
> section by section and diffing their citation sets. The Opus 5 side is a
> superset in substance: it carries a §0 scope boundary with `path:line` evidence,
> a severity-ranked findings table, an ordered commit sequence, and cross-feature
> requests that the other side does not. The branch side cited exactly one source
> the Opus 5 side missed, which has been pulled forward and is marked
> *(merged from the 4.8 spec)* where it appears.
>
> **This merged text is ungraded.** The scorecard on file graded the branch
> version. Treat the score as evidence that the feature was analysed, not as a
> verification of this document, and re-run Phase 2 before relying on it.
...
*(Merged from the 4.8 spec.)* This is not only a taste judgement — it is already
a recorded decision. `docs/plans/deferred-dashboard-notes.md:7` explicitly defers
any home-page activity section until Jeff owns the concept. So the answer to
"how does the hand-maintained 'In progress' list avoid going stale" is **not** to
build a feed that maintains itself; the live options are re-phrasing toward what
the lab durably *is*, or accepting a manual copy-review pass. Building the feed
would contradict a plan document, not just a criterion.
